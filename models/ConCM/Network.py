...
class PseudoTargetClassifier(nn.Module):
    ...
    def get_classifier_weights(self, uptil=-1):  # 获取分类器权重
        output = []
        for i, cls in enumerate(self.classifiers.children()):
    ...
    def forward(self,features,labels,attribute_feature_mean,phase="test",true_proto=[]):
        ...
        for i in range(features.shape[0]):
            ...
            if this_attributeid==[]:
                ...
                # bias prototype + attribute features
...
class ConCM(nn.Module):
    ...
    def base_knowledge_acquire(self,trainset={}):
        save_path=self.args.save_path+'/'+ self.args.dataset + '_feature_statistics.pt'
        if trainset=={}:
            mode="load"
        else:
    ...
    def train_ProtoComNet_5shot(self,trainset):
        ...
        save_path = self.args.save_path+ '/' + self.args.dataset + '_ProtoComNet' + '.pth'
        ...
        for epoch in tqdm_gen:
            ...
            loss.backward()
            optimizer.step()
            with torch.no_grad():  # Statistical Loss
                running_loss.add(loss.item())
    ...
    def train_ProtoComNet_1shot(self,trainset):
    ...
    def train_Projector(self, trainset,testset):
        ...
        save_path = self.args.save_path+'/'+self.args.dataset + '_session_' + str(0) + '.pth'
        self.eval()
        max_acc = 0
        ...
        optimizer = torch.optim.SGD([{'params': self.projector.parameters()}], lr=self.args.align_lr, momentum=0.9, dampening=0.9, weight_decay=1e-4)  # SGD随机梯度下降
        ...
        for epoch in tqdm_gen:
            ...
            for idx, batch in enumerate(train_loader):
                ...
                with torch.no_grad():
                    ...
                    tqdm_gen.write(out_string)
            scheduler.step()
            _,_,_,acc,_=self.model_test(testset)
            if acc>max_acc:
                max_acc = acc
                torch.save(self.state_dict(), save_path)
        # # save
    ...
    def increment_update(self,trainset,testset,novelset,session):
        novel_loader = DataLoader(dataset=novelset, batch_size=novelset.__len__(), shuffle=False, num_workers=self.args.num_workers,pin_memory=True)
        train_loader = DataLoader(dataset=trainset, batch_size=trainset.__len__(), shuffle=False, num_workers=self.args.num_workers,pin_memory=True)
        test_loader = DataLoader(dataset=testset, batch_size=self.args.test_batch, shuffle=False, num_workers=self.args.num_workers,pin_memory=True)
        save_path = self.args.save_path + '/' + self.args.dataset + '_session_' + str(session) + '.pth'
        ...
        if self.args.replay_num == self.args.shot:
            novel_feature_mean, novel_feature_cov, all_novel_feature_list, all_novel_label_list = self.get_novel_feature_statistic(train_loader, num_enhance=10)
            labels_list = range(self.args.base_class, self.args.base_class + session * self.args.way)
        else:
            _, _, all_novel_feature_list, all_novel_label_list = self.get_novel_feature_statistic(train_loader, num_enhance=10)
            novel_feature_mean, novel_feature_cov,_,_ = self.get_novel_feature_statistic(novel_loader,num_enhance=10)
        ...
        optimizer = torch.optim.SGD([{'params': self.projector.parameters()}], lr=self.args.increment_lr, momentum=0.9, dampening=0.9, weight_decay=1e-4)  # SGD随机梯度下降
        ...
        for epoch in tqdm_gen:
            ...
            for idx, batch in enumerate(Gtrain_loader):
                ...
                with torch.no_grad():  # Calculate the accuracy rate and loss
                    running_loss.add(loss.item())
                    running_acc.add(utils.count_acc(outputs, labels))
            ...
            for _ in range(num_enhance):
                ...
                for class_index in range(num_class):
                    ...
                    if features_this.size(0) > 1:
                        ...
                        cov_this = torch.zeros(features_this.size(1), features_this.size(1)).cuda()
                    cov_list.append(cov_this)
                cov = torch.stack(cov_list, dim=0)
    ...
    def calibration(self,novel_feature_mean,novel_feature_cov,labels):
        ...
        # cov
        fusion_cov_matrices = []
        for i in range(topk_weight.shape[0]):  # W 是 (5, M) 的张量
            w = topk_weight[i, :]  # 获取第 i 行的权重
            weighted_cov = torch.zeros(base_feature_cov.shape[1], base_feature_cov.shape[2]).cuda()
            for j in range(w.shape[0]):  # 遍历 M 个协方差矩阵
                weighted_cov += w[j] * base_feature_cov[topk_indices[i][j]]  # 对每个协方差矩阵加权并累加
            fusion_cov_matrices.append(weighted_cov)  # 存储加权后的结果
